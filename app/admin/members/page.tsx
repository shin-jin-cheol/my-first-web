  ...
  return (
    <section className="space-y-8">
      <header className="space-y-2">
        ...
        <p className="text-zinc-500 dark:text-zinc-300">{t(locale, "주인 계정에서 회원 아이디, 이메일, 인증 상태를 조회할 수 있습니다.", "Owner can view member IDs, emails, and verification status.")}</p>
      </header>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-zinc-100 dark:bg-zinc-900 text-zinc-600 dark:text-zinc-200">
            <tr>
              ...
              <th className="px-4 py-3">{t(locale, "이메일", "Email")}</th>
              <th className="px-4 py-3">{t(locale, "인증 상태", "Verified")}</th>
              <th className="px-4 py-3">{t(locale, "인증 방식", "Auth")}</th>
            ...
            {members.length === 0 ? (
              ...
              members.map((member) => (
                <tr key={member.id} className="border-t border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-200">
                  ...
                  <td className="px-4 py-3">{member.emailVerified ? "Yes" : "No"}</td>
