...
export default async function GuestBoardPage({ searchParams }: GuestBoardPageProps) {
  ...
  return (
    <section className="space-y-8">
      <header className="space-y-3">
        ...
        {session.role === "member" ? (
          <div>
            <Link
              href="/guest/new"
              className="inline-flex rounded-full border border-[var(--accent-mid)] bg-[var(--accent-primary)] px-4 py-2 text-sm font-semibold text-text-base shadow-[0_0_8px_rgb(from_var(--accent-primary)_r_g_b_/_0.12)] transition hover:-translate-y-0.5 hover:brightness-105"
            >
      ...
      {session.role === "owner" ? (
        <p className="rounded-xl border border-accent-border bg-accent-soft px-4 py-3 text-sm text-[var(--accent-dark)] shadow-[0_0_6px_rgb(from_var(--accent-primary)_r_g_b_/_0.08)] dark:text-accent-sub dark:shadow-none">
          {t(
            locale,
